      ...
      body: ListView(padding: const EdgeInsets.all(MoewSpacing.lg), children: [
        ...
        if (_pets.isEmpty)
          ...
          Container(
            ...
            child: DropdownButtonHideUnderline(child: DropdownButton<dynamic>(
              ...
              isExpanded: true,
              icon: const Icon(Icons.expand_more, color: MoewColors.textSub),
              items: _pets.map((p) => DropdownMenuItem(
                value: p['id'],
                child: Row(children: [
                  const Icon(Icons.pets, size: 18, color: MoewColors.secondary),
                  const SizedBox(width: 10),
                  Text(p['name'] ?? 'Pet #${p['id']}', style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600)),
                  if (p['breed'] != null) ...[const SizedBox(width: 8), Text('(${p['breed']})', style: MoewTextStyles.caption)],
                ]),
              )).toList(),
        ...
        SizedBox(
          ...
          child: ElevatedButton.icon(
            ...
            icon: _analyzing ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : const Icon(Icons.auto_awesome, color: Colors.white),
            label: Text(_analyzing ? 'Đang phân tích...' : 'Phân tích bằng AI', style: MoewTextStyles.button),
            style: ElevatedButton.styleFrom(backgroundColor: MoewColors.accent, padding: const EdgeInsets.symmetric(vertical: 16)),
          ),
        ),
        const SizedBox(height: MoewSpacing.lg),
        ...
        if (_result != null) ...[
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Container(width: 40, height: 40, decoration: BoxDecoration(color: MoewColors.tintPurple, borderRadius: BorderRadius.circular(MoewRadius.sm)), child: const Icon(Icons.auto_awesome, size: 20, color: MoewColors.accent)),
              ...
              if (_result!['goodIngredients'] is List && (_result!['goodIngredients'] as List).isNotEmpty) ...[
                ...
                Wrap(spacing: 6, runSpacing: 6, children: (_result!['goodIngredients'] as List).map<Widget>((g) => Container(
                  ...
                  child: Text(g.toString(), style: const TextStyle(fontSize: 12, color: MoewColors.success, fontWeight: FontWeight.w600)),
                )).toList()),
              ],
              // Bad ingredients
              if (_result!['badIngredients'] is List && (_result!['badIngredients'] as List).isNotEmpty) ...[
                ...
                Wrap(spacing: 6, runSpacing: 6, children: (_result!['badIngredients'] as List).map<Widget>((b) => Container(
                  ...
                  child: Text(b.toString(), style: const TextStyle(fontSize: 12, color: MoewColors.danger, fontWeight: FontWeight.w600)),
                )).toList()),
              ],
              // AI advice
          ...
          OutlinedButton.icon(
            ...
            icon: const Icon(Icons.chat_bubble_outline, color: MoewColors.accent),
            label: const Text('Hỏi AI thêm', style: TextStyle(color: MoewColors.accent, fontWeight: FontWeight.w700)),
            style: OutlinedButton.styleFrom(side: const BorderSide(color: MoewColors.accent), padding: const EdgeInsets.symmetric(vertical: 14)),
          ),
        ],
      ]),
    );
  ...
  void _showImagePicker() {
    showModalBottomSheet(
      ...
      builder: (ctx) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
        ListTile(leading: const Icon(Icons.camera_alt, color: MoewColors.primary), title: const Text('Chụp ảnh'), onTap: () { Navigator.pop(ctx); _pickImage(ImageSource.camera); }),
        ListTile(leading: const Icon(Icons.photo_library, color: MoewColors.secondary), title: const Text('Chọn từ thư viện'), onTap: () { Navigator.pop(ctx); _pickImage(ImageSource.gallery); }),
      ])),
    );
  }
