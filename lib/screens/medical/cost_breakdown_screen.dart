...
class _CostBreakdownScreenState extends State<CostBreakdownScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppHeader(title: 'Chi phí'),
      body: _loading
          ? Center(child: CircularProgressIndicator(color: MoewColors.primary))
          : Column(children: [
              // Total header
              Container(
                margin: EdgeInsets.all(MoewSpacing.lg),
                padding: EdgeInsets.all(MoewSpacing.lg),
                decoration: BoxDecoration(
                  gradient: LinearGradient(colors: [MoewColors.secondary, Color(0xFFE67E22)]),
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  ...
                  Text(formatVND(_total), style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800, color: Colors.white)),
                ]),
              ),
              Expanded(
                child: _costs.isEmpty
                    ? EmptyState(icon: Icons.attach_money, color: MoewColors.secondary, message: 'Chưa có chi phí nào')
                    : ListView.builder(
                          ...
                          return Container(
                            ...
                            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                              Expanded(child: Row(children: [
                                Container(width: 40, height: 40, decoration: BoxDecoration(color: MoewColors.tintAmber, borderRadius: BorderRadius.circular(MoewRadius.sm)), child: Icon(Icons.receipt_outlined, size: 20, color: MoewColors.secondary)),
                                SizedBox(width: 12),
                                Expanded(child: Text((c['description'] ?? c['name'] ?? '').toString(), style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: MoewColors.textMain), overflow: TextOverflow.ellipsis)),
                              ])),
                              Text(formatVND(c['amount']), style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: MoewColors.secondary)),
                            ]),
                          );
                        },
                      ),
      ...
      floatingActionButton: FloatingActionButton(onPressed: _addCost, backgroundColor: MoewColors.secondary, child: Icon(Icons.add, color: Colors.white)),
    );
  }
}
