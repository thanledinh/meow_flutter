...
class _SosScreenState extends State<SosScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _triggerSOS() async {
    ...
    if (res.success) {
      ...
      _mqttSub = MqttService().messageStream.listen((data) {
        ...
        if (type == 'sos_accepted' || type == 'sos:accepted') {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: _searching ? Color(0xFFFDEDEE) : MoewColors.background,
  ...
  Widget _buildForm() {
    return ListView(padding: EdgeInsets.all(MoewSpacing.lg), children: [
      // Warning banner
      Container(
      ...
      if (_pets.isEmpty)
        ...
        DropdownButtonFormField<dynamic>(
          ...
          decoration: InputDecoration(prefixIcon: Icon(Icons.pets, color: MoewColors.textSub)),
        ),
      SizedBox(height: MoewSpacing.md),

      Text('MÔ TẢ TÌNH TRẠNG', style: MoewTextStyles.label),
      SizedBox(height: MoewSpacing.sm),
      TextField(controller: _descCtrl, maxLines: 3, decoration: InputDecoration(hintText: 'Mô tả tình trạng thú cưng...')),
      ...
      SizedBox(
        ...
        child: ElevatedButton.icon(
          ...
          icon: _loading ? SizedBox(width: 22, height: 22, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : Icon(Icons.warning, size: 28, color: Colors.white),
          label: Text(_loading ? 'Đang gửi...' : 'GỬI SOS', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: Colors.white, letterSpacing: 2)),
    ...
    return Center(
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        OutlinedButton(
          ...
          style: OutlinedButton.styleFrom(side: BorderSide(color: MoewColors.danger)),
          child: Text('Hủy SOS', style: TextStyle(color: MoewColors.danger, fontWeight: FontWeight.w700)),
        ),
      ]),
    );
  }
